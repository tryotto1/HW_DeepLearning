...
class nn_mnist_classifier:
    ...
    def backprop(self):
        ...
        # similarly, fill in ... part in the below
        sm1_b = self.sm1.backprop(x=fc2_f, dLdy=cn_b)

        fc2_b, dldw_fc2, dldb_fc2 = self.fc2.backprop(x=ac2_f, dLdy=sm1_b)
        ac2_b = self.act_2.backprop(x=fc1_f, dLdy=fc2_b)

        fc1_b, dldw_fc1, dldb_fc1 = self.fc1.backprop(x=mp1_f, dLdy=ac2_b)
        mp1_b = self.maxpool_layer_1.backprop(x=ac1_f, dLdy=fc1_b)

        ac1_b = self.act_1.backprop(x=cv1_f, dLdy=mp1_b)
        cv1_b, dldw_cv1, dldb_cv1 = self.conv_layer_1.backprop(x=x, dLdy=ac1_b)
    ...
    def update_weights(self):
        ...
        self.v_w_fc2 = friction * self.v_w_fc2 - lr * dldw_fc2
        self.v_b_fc2 = friction * self.v_b_fc2 - lr * dldb_fc2

        self.v_w_fc1 = friction * self.v_w_fc1 - lr * dldw_fc1
        self.v_b_fc1 = friction * self.v_b_fc1 - lr * dldb_fc1

        self.v_w_cv1 = friction * self.v_w_cv1 - lr * dldw_cv1
        self.v_b_cv1 = friction * self.v_b_cv1 - lr * dldb_cv1

        # using v, perform weight update for each layer
        self.fc2.update_weights(dLdW=self.v_w_fc2, dLdb=self.v_b_fc2)
        self.fc1.update_weights(dLdW=self.v_w_fc1, dLdb=self.v_b_fc1)
        self.conv_layer_1.update_weights(dW=self.v_w_cv1, db=self.v_b_cv1)
